...
public class ItemSerieListaUso extends ItemSerieListaUsoBase {
	...
	protected List<ItemSerie> getListaCorrente(Context contexto, ItemSerieServico servico) {
		if (diaTreino==null) throw new RuntimeException("DiaTreino está null");
		if (diaTreino.getSerieTreino_SerieDia()==null) throw new RuntimeException("SerieTreino está null");
		servico.getFiltro().setDia(diaTreino);
		List<ItemSerie> lista = servico.ListaPorDiaComExecucao(getContext());
